...
- 可以反作用的模式 (shared_from_this)

  - 状态模式：状态反作用控制机器转换其它状态，机器创建状态对象时需要传入指向自身的指针

  - 原型模式：需要对自身的克隆，传入指向自身的指针

  - 访问者模式：引入访问者访问自身的接口

  - MVC 模式：创建视图时传入模型，模型再添加指向视图自身的观察者
...
- 将稳定数据结构和在该数据结构上的易变操作分离，更容易添加新的操作

- 组成：

  - 访问者接口：声明一系列访问方法，每个方法对应一个元素类

  - 具体访问者类：实现访问者接口

  - 元素接口：声明接受访问者的方法

  - 具体元素类：实现元素接口，每个具体元素对应一个数据结构中一个具体对象

- 例：实现一个菜单，一个访问者可以访问每个菜的同一个属性

  - 元素接受访问者，访问者访问元素

  - 比如：卡路里访问者统一访问所有菜的卡路里，健康率访问者统一访问所有菜的健康率
