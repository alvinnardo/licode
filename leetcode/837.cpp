...
double new21Game(int n, int k, int maxPts) {
    ...
    // 思路是从后往前推，最后推到 0
    // P(0胜) = P(1胜) * 1/W + P(2胜) * 1/W ...
    // *** 在 k ~ n 的胜率是 1，
    // *** (n + 1) ~ (n + w - n + k - 1) 即 (w + k - 1) 的胜率是 0

    // 注意：这里的 n 可能大于 k + maxPts
    // 大于的不用计算

    vector<double> dp(k + maxPts); // 0 ~ k + maxPts - 1
    int left = k, right = k + maxPts - 1;
    double tot = 0;
...
// 时间复杂度还是太高，范围是 10000, O(kn) 就是 1e8
